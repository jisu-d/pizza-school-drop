  ...
  const PizzaCanvas: React.FC = () => {
    ...
    const gravityRef = useRef({ x: 0, y: 0.25 });
    ...
    useEffect(() => {
      ...
      // 화면 방향 잠금 (지원되는 브라우저에서)
      // if (screen.orientation && screen.orientation.lock) {
      //   screen.orientation.lock('portrait').catch((err) => console.error('Orientation lock failed:', err));
      // }
      // 화면 회전 시 경고
    ...
    const handleDeviceOrientation = (e: DeviceOrientationEvent) => {
      ...
      const shakeThreshold = 20; // 기울기 임계값 (더 민감하게 조정)
      const shakeAmplifier = 2; // 흔들기 가속도 배율
      ...
      gravityRef.current = { x: x * 0.9, y: y * 0.9 }; // 부드러운 중력 스케일링
    };
